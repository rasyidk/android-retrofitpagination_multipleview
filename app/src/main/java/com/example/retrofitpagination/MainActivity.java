...
//        swipeRefreshLayout = findViewById(R.id.main_swiperefresh);
        ...
        rv.addOnScrollListener(new PaginationScrollListener(linearLayoutManager) {
            ...
            protected void loadMoreItems() {
                ...
                SharedPreferences.Editor editor = getSharedPreferences("page2", MODE_PRIVATE).edit();
                editor.putString("cur",cupage.toString());
                editor.apply();
...
//                Log.i(TAG, "onResponse: " + (response.raw().cacheResponse() != null ? "Cache" : "Network"));
                Integer cup = currentPage;
                Log.d("STATUS", cup.toString());
                Log.d("RESULT", topRatedMovies.getPage());



                try{
                    if (cup == Integer.parseInt(topRatedMovies.getPage())){
                        Log.d("OPERATOR","TRUEEE");
                        List<Result> results = fetchResults(response);
                        progressBar.setVisibility(View.GONE);
                        adapter.addAll(results);
                    }else {
                        Log.d("OPERATOR","FALSE");
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
